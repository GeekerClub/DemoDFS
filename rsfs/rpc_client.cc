...
DEFINE_string(rsfs_rpcclient_log_level, "ERROR", "");

DEFINE_bool(rsfs_rpcclient_traffic_limit_enabled, false, "");
DEFINE_int32(rsfs_rpcclient_traffic_limit_max_inflow, 10,
             "max in-traffic bandwidth (in MB/s)");
DEFINE_int32(rsfs_rpcclient_traffic_limit_max_outflow, 10,
             "max out-traffic bandwidth (in MB/s)");

DEFINE_int32(rsfs_rpcclient_max_pending_buffer_size, 2,
             "max pending buffer size (in MB) for server");
DEFINE_int32(rsfs_rpcclient_work_thread_num, 4, "");
