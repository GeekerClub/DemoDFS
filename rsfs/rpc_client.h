...
DECLARE_string(rsfs_rpcclient_log_level);
DECLARE_bool(rsfs_rpcclient_traffic_limit_enabled);
DECLARE_int32(rsfs_rpcclient_traffic_limit_max_inflow);
DECLARE_int32(rsfs_rpcclient_traffic_limit_max_outflow);
DECLARE_int32(rsfs_rpcclient_max_pending_buffer_size);
DECLARE_int32(rsfs_rpcclient_work_thread_num);
...
class RpcClientBase {
public:
    static void SetOption(int32_t max_inflow = FLAGS_rsfs_rpcclient_traffic_limit_max_inflow,
                          int32_t max_outflow = FLAGS_rsfs_rpcclient_traffic_limit_max_outflow,
                          int32_t pending_buffer_size = FLAGS_rsfs_rpcclient_max_pending_buffer_size,
                          int32_t thread_num = FLAGS_rsfs_rpcclient_work_thread_num,
                          std::string log_level = FLAGS_rsfs_rpcclient_log_level) {
        if (log_level == "NOTICE") {
        ...
        if (FLAGS_rsfs_rpcclient_traffic_limit_enabled
            && -1 != max_inflow) {
...
protected:
    ...
    bool SendMessageWithRetry(void(ServerType::*func)(
        ...
        toft::scoped_ptr<trident::RpcController> rpc_controller;
        rpc_controller.reset(new trident::RpcController);
        rpc_controller->SetTimeout(rpc_timeout);

        google::protobuf::Closure* done = NULL;
        if (closure != NULL) {
            RpcCallbackParam<Request, Response, Callback>* param =
                new RpcCallbackParam<Request, Response, Callback>(
                    rpc_controller.get(), request, response,
                    closure, tips, thread_pool);
            done = google::protobuf::NewCallback(
                &RpcClient::template RpcCallback<Request, Response, Callback>,
                param);
            (m_server_client.get()->*func)(rpc_controller.release(),
