...
DEFINE_string(rsfs_rpcserver_log_level, "ERROR", "");

DEFINE_bool(rsfs_rpcserver_traffic_limit_enabled, false, "");
DEFINE_int32(rsfs_rpcserver_traffic_limit_max_inflow, 10,
             "max in-traffic bandwidth (in MB/s)");
DEFINE_int32(rsfs_rpcserver_traffic_limit_max_outflow, 10,
             "max out-traffic bandwidth (in MB/s)");

DEFINE_int32(rsfs_rpcserver_retry_times, 3,
             "the retry times when server meets exception");
DEFINE_int32(rsfs_rpcserver_retry_period, 1000,
             "the retry period (in ms) between both retry operation");
DEFINE_int32(rsfs_rpcserver_error_notify_interval, 2000,
             "the interval (in ms) between two error notification");
DEFINE_int32(rsfs_rpcserver_error_retry_times, 15,
             "the maximal retry times of same error");
DEFINE_int32(rsfs_rpcserver_keep_alive_time, 1800,
             "the maximal interval (in sec) to keep idle connections");
DEFINE_int32(rsfs_rpcserver_max_pending_buffer_size, 2,
             "max pending buffer size (in MB) for server");
DEFINE_int32(rsfs_rpcserver_work_thread_num, 4, "");
...
bool RpcServer::StopServer() {
    ...
    if (IsJoinable()) {
        ...
        VLOG(10) << "rsfs server is not started";
    }
    return true;
}
    ...
    if (error_code == trident::RPC_ERROR_TOO_MANY_OPEN_FILES) {
        ...
        if (m_error_retry == 0
            || cur_timestamp > m_error_previous_timestamp
                + FLAGS_rsfs_rpcserver_error_notify_interval) {
            m_error_retry = 1;
        } else {
            m_error_retry++;
        }
        m_error_previous_timestamp = cur_timestamp;
        CHECK((int32_t)m_error_retry < FLAGS_rsfs_rpcserver_error_retry_times)
...
RpcServerTrident::RpcServerTrident(const std::string& ip, uint16_t port)
    : m_address(ip, port), m_started(false),
      m_server(NULL) {
    if (FLAGS_rsfs_rpcserver_log_level == "NOTICE") {
        TRIDENT_SET_LOG_LEVEL(NOTICE);
    } else {
        TRIDENT_SET_LOG_LEVEL(ERROR);
    }
    if (FLAGS_rsfs_rpcserver_traffic_limit_enabled) {
        m_options.max_throughput_in =
            FLAGS_rsfs_rpcserver_traffic_limit_max_inflow;
        m_options.max_throughput_out =
            FLAGS_rsfs_rpcserver_traffic_limit_max_outflow;
    }
    m_options.max_pending_buffer_size =
        FLAGS_rsfs_rpcserver_max_pending_buffer_size;
    m_options.work_thread_num = FLAGS_rsfs_rpcserver_work_thread_num;
    m_options.keep_alive_time = FLAGS_rsfs_rpcserver_keep_alive_time;
    ...
    LOG(INFO) << "RPC server is activated (log level: "
        << FLAGS_rsfs_rpcserver_log_level
...
void RpcServerTrident::Loop() {
    uint32_t wait_time = FLAGS_rsfs_rpcserver_retry_period;
    for (int32_t retry = 0; !m_started && retry < FLAGS_rsfs_rpcserver_retry_times;
         ++retry) {
